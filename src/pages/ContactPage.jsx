...
export default function ContactPage() {
  ...
  const openWhatsApp = () =>
    window.open(
      `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(buildWaText())}`,
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setStatus('success');
      // Also open WhatsApp so admin gets an instant ping
      openWhatsApp();
      setTimeout(() => {
        setStatus('idle');
      ...
      // Decode error shape from client.js
      if (err.errors?.length) {
        setErrorMsg(err.errors.map(e => (typeof e === 'string' ? e : e.message)).join(', '));
      } else if (err.data?.message) {
        setErrorMsg(err.data.message);
      } else {
        setErrorMsg(err.message || 'Failed to send. Please try WhatsApp below.');
      }
    }
  };
  ...
  const clickableValueStyle = {
    ...
    color: '#c8b89a', fontSize: '1rem', textDecoration: 'none',
    transition: 'color 0.2s ease', cursor: 'pointer', display: 'inline-block',
  };

  const WaIcon = ({ size = 16, color = '#4ade80' }) => (
        ...
        <div style={{
          ...
          <div>
            ...
            {CONTACT_DETAILS.map(({ icon, label, value, href }, i) => (
              <div key={i} style={{ display: 'flex', gap: '1rem', marginBottom: '1.2rem', alignItems: 'flex-start' }}>
                ...
                <div>
                  ...
                  {href ? (
                    <a href={href} target="_blank" rel="noopener noreferrer" style={clickableValueStyle}
                      onMouseEnter={e => e.currentTarget.style.color = '#D4AF37'}
                      onMouseLeave={e => e.currentTarget.style.color = '#c8b89a'}>
                      {value}
                    </a>
                  ) : (
                    <div style={{ fontFamily: "'Cormorant Garamond', serif", color: '#c8b89a', fontSize: '1rem' }}>{value}</div>
                  )}
                </div>
              </div>
            ))}
            ...
            <a href={`https://wa.me/${WHATSAPP_NUMBER}`} target="_blank" rel="noopener noreferrer"
              style={{
                ...
                fontFamily: "'Cormorant Garamond', serif", color: '#4ade80', fontSize: '0.9rem',
            ...
            <div style={{ marginTop: '1.5rem', padding: '1.5rem', background: 'rgba(212,175,55,0.05)', border: '1px solid rgba(212,175,55,0.12)', borderRadius: '12px' }}>
          ...
          <form onSubmit={submit}>
            ...
            <p style={{ fontFamily: "'Cormorant Garamond', serif", color: '#a89060', fontSize: '0.85rem', marginBottom: '1.4rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <WaIcon size={13} />
              Saved to our system &amp; sent directly to WhatsApp
            </p>
            ...
            {status === 'error' && (
              <div style={{
                ...
                fontFamily: "'Cormorant Garamond', serif", color: '#fca5a5', fontSize: '0.95rem',
              }}>
                ⚠️ {errorMsg}{' '}
                <button type="button" onClick={openWhatsApp}
                  style={{ background: 'none', border: 'none', color: '#4ade80', cursor: 'pointer', padding: 0, fontFamily: 'inherit', fontSize: 'inherit', textDecoration: 'underline' }}>
            ...
            {status === 'success' && (
              <div style={{
                ...
                borderRadius: '8px', padding: '0.75rem 1rem', marginBottom: '1.2rem',
                fontFamily: "'Cormorant Garamond', serif", color: '#86efac', fontSize: '0.95rem',
              }}>
                ✅ Message saved! WhatsApp is opening so we get your message instantly.
              </div>
            )}
            ...
            <button
              ...
              disabled={status === 'loading' || status === 'success'}
              style={{
                ...goldBtnStyle, width: '100%', justifyContent: 'center',
                padding: '0.9rem', borderRadius: '8px', gap: '0.5rem',
            ...
            >
              {status === 'loading' ? '⏳ Sending…' : status === 'success' ? (
                <><WaIcon size={15} /> Opening WhatsApp…</>
              ) : (
                <><WaIcon size={15} /> Send Message</>
              )}
            </button>
